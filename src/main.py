...
def run_scan():
    ...
    try:
        ...
        if not symbols:
            logger.info("No active symbols found. Please add symbols to the database.")
            # Seeding Nifty 50 (India) Tickers
            # Yahoo Finance uses .NS suffix for NSE
            nifty_50_tickers = [
                "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "INFY.NS", "ICICIBANK.NS",
                "HINDUNILVR.NS", "ITC.NS", "SBIN.NS", "BHARTIARTL.NS", "KOTAKBANK.NS",
                "LTIM.NS", "AXISBANK.NS", "BAJFINANCE.NS", "LT.NS", "MARUTI.NS",
                "HCLTECH.NS", "TITAN.NS", "ASIANPAINT.NS", "SUNPHARMA.NS", "ULTRACEMCO.NS",
                "BAJAJFINSV.NS", "SBILIFE.NS", "HDFCLIFE.NS", "ICICIPRULI.NS",
                "NTPC.NS", "ONGC.NS", "POWERGRID.NS", "BPCL.NS", "COALINDIA.NS",
                "TATASTEEL.NS", "JSWSTEEL.NS", "GRASIM.NS", "HINDALCO.NS",
                "NESTLEIND.NS", "BRITANNIA.NS", "TATACONSUM.NS",
                "BAJAJAUTO.NS", "M&M.NS", "HEROMOTOCO.NS",
                "ADANIENT.NS", "ADANIPORTS.NS",
                "DRREDDY.NS", "APOLLOHOSP.NS",
                "WIPRO.NS", "TECHM.NS",
                "INDUSINDBK.NS", "CIPLA.NS"
            ]
            logger.info(f"Seeding {len(nifty_50_tickers)} Nifty 50 symbols...")
            for ticker in nifty_50_tickers:
                market_data_service.fetch_and_store(ticker) 
            symbols = db.query(Symbol).filter(Symbol.is_active == True).all()
        ...
        for symbol in symbols:
            try:
                ...
                if result['confidence'] in ["High", "Medium", "Low"]:
                    ...
                    msg += f"<i>Generated by Tranforge Solutions LLP</i>"
